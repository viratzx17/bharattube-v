...
export function CollectionPage({
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const endpoint = feed === "liked"
        ? "/liked-videos/liked-videos?limit=50"
        : `/videos?feed=${feed}`;
      const res = await fetch(apiUrl(endpoint), { cache: "no-store", credentials: "include" });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || data?.message || "Failed to load");
      setVideos(adaptVideos(feed === "liked" ? { videos: data?.videos ?? data?.data?.videos ?? [] } : data) as unknown as VideoItem[]);
    } catch {
