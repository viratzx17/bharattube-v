...
function MyVideosContent() {
  ...
  const saveEdit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const res = await fetch(apiUrl(`/videos/${encodeURIComponent(editing.id)}`), {
        method: "PUT",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        showToast(data?.message || data?.error || "Could not save changes", "error");
        return;
      }
      ...
      setVideos((prev) =>
        prev.map((v) =>
          v.id === editing.id
            ? {
                ...v,
                title: data?.data?.title || data?.video?.title || editTitle.trim(),
                description: data?.data?.description ?? data?.video?.description ?? editDescription.trim(),
