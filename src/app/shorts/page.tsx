...
function ShortSlide({
  ...
  useEffect(() => {
    ...
    if (active) {
      ...
      if (audio) { audio.currentTime = 0; audio.muted = muted; }
      // Start unmuted. Browsers may reject autoplay with audio; do not silently
      // switch the user's volume off. Instead keep the element unmuted and let
      // the first tap retry playback with sound.
      v.muted = Boolean(short.musicUrl) ? true : muted;
      if (audio && short.musicUrl) audio.muted = muted;
      const playVideo = v.play();
      playVideo.then(() => {
        setSoundBlocked(false);
        setPaused(false);
        if (audio && short.musicUrl) audio.play().catch(() => setSoundBlocked(true));
      }).catch(() => {
  ...
  useEffect(() => {
    const v = videoRef.current;
    const audio = audioRef.current;
    if (v) v.muted = Boolean(short.musicUrl) ? true : muted;
    if (audio) audio.muted = muted;
    const track = captionTrackRef.current?.track;
    if (track) track.mode = captionsEnabled ? "showing" : "hidden";
  }, [muted, short.musicUrl, captionsEnabled, captionSrc]);

  useEffect(() => {
    if (!short.captionsVtt) { setCaptionSrc(""); return; }
    const url = URL.createObjectURL(new Blob([short.captionsVtt], { type: "text/vtt;charset=utf-8" }));
    setCaptionSrc(url);
  ...
  const togglePlay = () => {
    ...
    if (v.paused) {
      v.muted = Boolean(short.musicUrl) ? true : muted;
      v.play()
        .then(() => { setSoundBlocked(false); setPaused(false); if (audioRef.current && short.musicUrl) audioRef.current.play().catch(() => setSoundBlocked(true)); })
  ...
  const handleLike = async () => {
    ...
    setBusyLike(true);
    try {
      const removing = short.userReaction === "like";
      const res = await fetch(apiUrl(`/likes/${encodeURIComponent(String(short.id))}`), {
      ...
      if (!res.ok) {
        showToast(data?.message || data?.error || "Could not update this like", "error");
        return;
      }
      onLiked(Number(data.likes ?? data.likesCount ?? short.likesCount), removing ? null : "like");
    } catch {
  ...
  return (
    <section className="snap-item relative h-[calc(100dvh-7rem-env(safe-area-inset-bottom,0px))] md:h-[calc(100dvh-3.5rem)] w-full flex items-center justify-center bg-black">
      <div className="relative h-full aspect-[9/16] max-w-full overflow-hidden bg-black sm:rounded-2xl">
      <video
        ref={videoRef}
        src={short.videoUrl}
      ...
      >
        {short.captionsVtt && <track ref={captionTrackRef} kind="subtitles" src={captionSrc} srcLang="hi" label="Captions" />}
      </video>
      {short.musicUrl && <audio ref={audioRef} src={short.musicUrl} loop preload={active ? "auto" : "none"} />}
      </div>
      ...
      {soundBlocked && active && (
        <button
          ...
          onClick={() => {
            const v = videoRef.current;
            if (!v) return;
            v.muted = Boolean(short.musicUrl) ? true : false;
            if (audioRef.current && short.musicUrl) audioRef.current.muted = false;
            void v.play().then(() => {
              setSoundBlocked(false);
              setPaused(false);
              if (audioRef.current && short.musicUrl) void audioRef.current.play();
            });
          }}
          className="absolute inset-x-0 top-16 mx-auto w-fit rounded-full bg-black/75 px-4 py-2 text-xs font-semibold text-white backdrop-blur-md"
        >
          Tap to enable sound
        </button>
      )}
      ...
      <button type="button" onClick={() => { if (!short.captionsVtt) { showToast("No captions are available for this Short.", "info"); return; } setCaptionsEnabled((v) => !v); }} aria-label="Captions" className={`absolute top-3 right-14 tap-target rounded-full flex items-center justify-center backdrop-blur-sm ${captionsEnabled ? "bg-red-600" : "bg-black/60"} text-white`}><Subtitles className="w-5 h-5" /></button>
