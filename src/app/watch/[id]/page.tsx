...
export default function WatchPage({
  ...
  const fetchWatchData = useCallback(async () => {
    ...
    try {
      ...
      const userReaction =
        rawVideo?.userReaction === "like" || rawVideo?.userReaction === "dislike"
          ? rawVideo.userReaction
          : null;
      ...
      const channelId = rawChannel?._id;
      if (channelId) {
        ...
        if (subRes.ok) {
          ...
          setVideo((current) =>
            current
              ? {
                  ...current,
                  isSubscribed: Boolean(subPayload?.isSubscribed ?? subPayload?.data?.isSubscribed),
                  creator: {
                    ...current.creator,
  ...
  /** Comments use the existing Mongo/Express comment routes. */
  const fetchComments = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        setCommentsUnsupported(isUnsupportedResponse(payload));
        setComments([]);
        return;
      }
      ...
      const mapComment = (raw: any): CommentData => ({
        ...
        parentId: raw?.parentComment ? String(raw.parentComment) : null,
      ...
      // Network problem: keep the current comments visible.
    }
  }, [videoId, user?.id]);
      ...
      >
        ...
        <div className={theaterMode ? "w-full" : "lg:col-span-8"}>
          ...
          <div className="mt-3 px-3 sm:px-0 flex flex-wrap items-center justify-between gap-3 pb-4 border-b border-zinc-200 dark:border-zinc-800">
            ...
            {/* Video Action Pills */}
            <div className="flex w-full sm:w-auto max-w-full items-center gap-2 -mx-3 px-3 sm:mx-0 sm:px-0 pb-1 overflow-x-auto no-scrollbar snap-x">
              <LikeDislikePill
                videoId={video.id}
              ...
              <div className="relative shrink-0">
                <button
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2.5 rounded-full bg-zinc-200/80 dark:bg-zinc-800/90 hover:bg-zinc-300/70 dark:hover:bg-zinc-700 active:scale-95 text-zinc-900 dark:text-zinc-100 text-sm font-medium transition-all cursor-pointer"
                >
                  <Share2 className="w-4 h-4" />
                  <span>Share</span>
                </button>
                {shareMenuOpen && (
                  <div className="absolute z-40 top-full left-0 mt-2 w-48 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 shadow-2xl p-1.5">
                    <button type="button" onClick={async () => { setShareMenuOpen(false); await handleShare(); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-left hover:bg-zinc-100 dark:hover:bg-zinc-800">
                      <Share2 className="w-4 h-4" /> Share / Copy link
                    </button>
                    <button type="button" onClick={() => { setShareMenuOpen(false); handleWhatsAppShare(); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 text-emerald-600">
