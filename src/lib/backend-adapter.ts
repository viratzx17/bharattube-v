...
export function adaptVideo(raw: unknown): AdaptedVideo | null {
  ...
  const channel = (v.channel && typeof v.channel === "object" ? v.channel : {}) as Record<string, any>;
  const channelSubscribers = Array.isArray(channel.subscribers)
    ? channel.subscribers.length
    : num(channel.subscribersCount, channel.subscriberCount);

  const creator = {
    // Subscription APIs are keyed by the channel _id, not the owner user id.
    id: str(channel._id, v.channelId, ownerIsObject ? owner._id : owner, v.ownerId, v.userId),
    channelId: str(channel._id, v.channelId),
    username: str(
      v.channelHandle,
      channel.handle,
    ...
    avatarUrl: str(channel.logo, ownerIsObject ? owner.profilePhoto : "") || null,
    isVerified: Boolean(channel.verified ?? (ownerIsObject && owner.verified)),
    subscriberCount: channelSubscribers ?? 0,
  };
  ...
  return {
    ...
    captionsVtt: str(v.captionsVtt, v.captions, v.captionText),
    musicUrl: str(v.musicUrl, v.music),
    userReaction: v.userReaction === "like" || v.userReaction === "dislike" ? v.userReaction : null,
